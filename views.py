...
def studentdetail_search(request):
    if request.method == 'GET':
        if 'regno' in request.GET or 'cursem' in request.GET or 'branch' in request.GET:
            ...
            print("sss ", marklist[0].subcodel1)
            # subl1 = Subject_Profile.objects.filter(code=marklist[1].subcodel1)
            # subl2 = Subject_Profile.objects.filter(code=marklist[1].subcodel2)
            #
            # subl3 = Subject_Profile.objects.filter(code=marklist[1].subcodel3)
...
from .filters import AttendenceFilter,MarklistFilter
...
def facultysubject_search(request):
    if request.user.groups.filter(name='Dataop') or request.user.groups.filter(name='Admin') or request.user.groups.filter(name='Faculty'):
...
@login_required()
def Facultyupdate(request, empid_id):
    if request.user.groups.filter(name='Dataop').exists() or request.user.groups.filter(name='Admin').exists():
        instance = get_object_or_404(Faculty, empid=empid_id)
        faculty = Facultymodelform(request.POST or None, instance=instance)
        if faculty.is_valid():
            instance = faculty.save(commit=False)
            instance.save()
...
def MarklistCreate(request):
    if request.user.groups.filter(name='Dataop') or request.user.groups.filter(
        ...
        # print("----------------",request.POST)
        marklist = MarklistForm(request.user, request.POST or None)
        # Student.objects.values_list('regno', flat=True))
        # marklist.fields['regno'].queryset = Student.objects.values_list('regno', flat=True).filter(
        #     branch=request.user.dept)
        # print("id of form ",marklist.id)
        # print("id of fields ",marklist.fields['cursem'].id)
        # print(request)
...
def attendence_list_pdf(request):
    if request.method == 'GET':
        ...
        list = RollnoRegnoMap.objects.filter(branch=branch, cursem=cursem, section=section,
                                             facultyid=facultyid, subjectcode=subcode).order_by('cursem', 'rollno','time')
